...
class TestPVMapsRealDataMulti(TestPVMaps):
    ...
    def test_pv_output(self):
        ...
        **Note** cached api data is stored in REAL_DATA_INPUT_DIR in api_real_pv_output.pkl - this will only be
        ...
        """
        def _dotest(name: str, exp_max_error: float):
            print(f"Running {name}")

            self.init_dirs(name)

            self._run_pvmaps(flat_roof_degrees=self.FLAT_ROOF_DEGREES, flat_roof_degrees_threshold=self.FLAT_ROOF_DEGREES_THRESHOLD)
        ...
        self.parameterised_test([
            ("thurso", 3.91, None),       # Missing pvmaps solar spectral and wind data
            ("alnwick", 1.81, None),      # Missing pvmaps solar spectral data
            ("alnwick_flat", 1.91, None),  # Flat roof (& missing pvmaps solar spectral data)
        ], _dotest)
