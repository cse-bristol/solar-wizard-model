...
WITH irr_bands AS (
    -- Generate 30 ranges, from [0.00,0.01) to [0.29,0.30)
    SELECT numrange(a / 100.0, a / 100.0 + 0.01, '[)') AS irr_band
...
FROM
    irr_bands
    LEFT JOIN models.pv_cost_benefit_best cb ON irr_bands.irr_band @> cb.irr::numeric
WHERE
    cb.job_id = %(job_id)s
