...
from solar_model.paths import SQL_DIR
...
def process_pg_uri(pg_uri: str) -> str:
    ...
    if parsed.scheme == '':
        ...
        return pg_uri + ' ' + 'application_name=solar'

    if len(parsed.query) == 0:
        return parsed._replace(query='application_name=solar').geturl()
    else:
        return parsed._replace(query=f'{parsed.query}&application_name=solar').geturl()
